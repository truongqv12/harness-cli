import fs from 'node:fs';
import { hookCommandValues, referencesExcludedHook, referencesManagedHook } from './hook-migration-compatibility.mjs';

function shouldPruneHook(hook) {
  return hookCommandValues(hook).some((value) => referencesManagedHook(value) || referencesExcludedHook(value));
}

export function mergeHooksJson(existingPath, generatedHooks) {
  const current = fs.existsSync(existingPath) ? JSON.parse(fs.readFileSync(existingPath, 'utf8')) : {};
  const hooks = current.hooks && typeof current.hooks === 'object' ? current.hooks : {};
  const next = {};
  for (const [event, groups] of Object.entries(hooks)) {
    next[event] = (Array.isArray(groups) ? groups : [])
      .map((group) => ({
        ...group,
        hooks: (group.hooks || []).filter((hook) => !shouldPruneHook(hook))
      }))
      .filter((group) => group.hooks.length);
  }
  for (const [event, groups] of Object.entries(generatedHooks)) {
    next[event] = [...(next[event] || []), ...groups];
  }
  return `${JSON.stringify({ ...current, hooks: next }, null, 2)}\n`;
}
