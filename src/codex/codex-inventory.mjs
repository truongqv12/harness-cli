...
function copyTree(files, sourceDir, tempRoot, targetBase) {
  ...
  for (const sourcePath of walkFiles(sourceDir)) {
    ...
    const targetRel = joinRel(targetBase, rel);
...
function buildHooks(files, sourceRoot, tempRoot, projectRoot) {
  ...
  copyTree(files, path.join(sourceRoot, 'claude', 'hooks'), tempRoot, '.codex/hooks/claude');
  ...
  for (const [event, groups] of Object.entries(settings.hooks || {})) {
    ...
    groups.forEach((group, groupIndex) => {
      ...
      (group.hooks || []).forEach((hook, hookIndex) => {
        const match = String(hook.command || '').match(/\.claude\/hooks\/([^"]+)/);
        if (!match || hook.type !== 'command') return;
        const scriptRel = normalizeRel(match[1]);
        ...
        handlers.push({
          ...
          commandWindows: `powershell -NoProfile -ExecutionPolicy Bypass -Command "$r=(git rev-parse --show-toplevel 2>$null); if (-not $r) { $r=(Get-Location).Path }; node \\"$r/.codex/hooks/vnpt-managed/${name}\\""`,
          statusMessage: hook.statusMessage || `VNPT ${event} hook`
        });
      });
      if (handlers.length) generated[event].push({ ...(group.matcher ? { matcher: group.matcher } : {}), hooks: handlers });
    });
  }
